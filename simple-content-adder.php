<?php
/*
Plugin Name: Simple Content Adder 
Plugin URI: https://wordpress.org/plugins/simple-content-adder/
Description: Add custom content to your posts, pages and/or footer, without the need to update each post or page. 
Version: 1.0
Author: Blaz K. 
License: GPL2
License URI: https://www.gnu.org/licenses/gpl-2.0.html
Text Domain: simple-content-adder 
*/
...
add_filter('manage_edit-sca_content_columns','sca_column_headers');

//register custom columns data

add_filter('manage_sca_content_posts_custom_column','sca_column_data',1,2);

//advanced custom fields settings 

add_filter('acf/settings/path', 'sca_acf_settings_path');
add_filter('acf/settings/dir', 'sca_acf_settings_dir');
add_filter('acf/settings/show_admin', 'sca_acf_show_admin');
if( !defined('ACF_LITE') ) define('ACF_LITE',true);

//register custom menus

add_action('admin_menu', 'sca_admin_menus');

//load js in the admin area

add_action('admin_enqueue_scripts', 'sca_admin_scripts'); 

//add added content to the existing content 

add_filter('the_content', 'sca_add_to_content',20);

//add added content to the footer 

add_action('wp_footer', 'sca_add_to_footer');
...
function sca_column_headers ( $columns ) {
...
function sca_column_data( $column, $post_id ) {
	...
	switch( $column ) {
        ...
        case 'included':
			...
			$included = get_field('sca_include', $post_id );
		...
		case 'position':
			...
			$position = get_field('sca_position', $post_id);
        ...
        case 'customclass':
			...
			$scaclassWS = get_field('sca_class', $post_id );
...
function sca_admin_menus() {
	
		$top_menu_item = 'sca_dashboard_admin_page'; 
	    
	    add_menu_page( '', 'Added Content', 'manage_options', 'sca_dashboard_admin_page', 'sca_dashboard_admin_page', 'dashicons-welcome-add-page' );
    
	    add_submenu_page( $top_menu_item, '', 'Dashboard', 'manage_options', $top_menu_item, $top_menu_item );
	    
	    add_submenu_page( $top_menu_item, '', 'Added Content', 'manage_options', 'edit.php?post_type=sca_content' );
	    
	    add_submenu_page( $top_menu_item, '', 'Add New', 'manage_options', 'post-new.php?post_type=sca_content' );
...
function sca_add_to_content ( $content ) {
         
         $content_query = new WP_Query( 
			array(
				'post_type' => 'sca_content',
				'published' => true,
				'orderby'=>'post_date',
				'order'=>'DESC',
			)
	...
	if( isset($content_query) && $content_query->have_posts() ):
		...
		while ($content_query->have_posts() ) : 
            ...
            $addedContentContent = get_field( "sca_content", $post_id );
         
            $addedContentPosition = get_field( "sca_position", $post_id );
    
            $addedContentInclude = get_field( "sca_include", $post_id ); 
    
            $addedContentClassWS = get_field( "sca_class", $post_id ); 
    
            $addedContentFront = get_field( "sca_front", $post_id );
...
function sca_dashboard_admin_page() {
